"""Step 4: deep-learning lane following with an explicit drive safety gate."""
    ...
    try:
        while max_frames is None or valid_frames < max_frames:
            ...
            if not lane_visible:
                ...
                consecutive_ready += 1
                consecutive_lost = 0
                predicted_angle, preview_frame = deep_detector.follow_lane(frame)
                command_angle = safe_servo_angle(predicted_angle, servo_offset)
                servo.set_angle(command_angle)
            ...
            if preview:
                cv2.imshow('deep lane follower', preview_frame)
...
def build_argument_parser():
    ...
    parser.add_argument('--speed', type=speed_percentage, default=37)
